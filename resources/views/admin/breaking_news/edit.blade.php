...
<div class="box-heading mb-4">
    <div class="box-title">
        <h3 class="mb-1">Edit Breaking News</h3>
        <p class="text-muted mb-0 font-sm">Modify the ticker content or toggle active status.</p>
    </div>
    <div class="box-breadcrumb">
        <div class="breadcrumbs">
            <ul>
                ...
                <li><span>Edit Breaking News</span></li>
            </ul>
        </div>
    </div>
</div>
    ...
    <div class="col-lg-9 col-md-11 mb-4">
        <div class="panel-white" style="border: 1px solid var(--border-color); border-radius: 16px; background: var(--card-bg); padding: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
            <div class="panel-body">
                <form action="/admin/breaking-news/{{ $breakingNews->id }}" method="POST" id="breaking-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="breaking-title" class="form-control w-100 @error('title') is-invalid @enderror" placeholder="Enter breaking news text here..." style="height: 46px; border-radius: 8px; font-size: 14px;" value="{{ old('title', $breakingNews->title) }}" required>
                        @error('title')
                    ...
                    <div class="p-3 rounded-3 mb-4 border" style="background: #F8FAFC; border-color: #BFDBFE !important;">
                        ...
                        <div class="tab-content bg-white p-3 rounded-3 border" style="border-color: #E2E8F0;">
                            ...
                            <div class="tab-pane fade show active" id="tab-en" role="tabpanel">
                                ...
                                <input type="text" name="title_en" id="breaking-title-en" class="form-control font-sm" placeholder="English alert headline..." value="{{ $breakingNews->title_en }}">
                            </div>
                            ...
                            <div class="tab-pane fade" id="tab-hi" role="tabpanel">
                                ...
                                <input type="text" name="title_hi" id="breaking-title-hi" class="form-control font-sm" placeholder="हिंदी में ब्रेकिंग न्यूज़ हेडलाइन..." value="{{ $breakingNews->title_hi }}">
                            </div>
                            ...
                            <div class="tab-pane fade" id="tab-pb" role="tabpanel">
                                ...
                                <input type="text" name="title_pb" id="breaking-title-pb" class="form-control font-sm" placeholder="ਪੰਜਾਬੀ ਵਿੱਚ ਬ੍ਰੇਕਿੰਗ ਨਿਊਜ਼ ਸਿਰਲੇਖ..." value="{{ $breakingNews->title_pb }}">
                            </div>
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="is_active" class="form-control w-100" style="height: 46px; border-radius: 8px;">
                            <option value="1" {{ $breakingNews->is_active ? 'selected' : '' }}>Active (Ticker Visible)</option>
                            <option value="0" {{ !$breakingNews->is_active ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        if ($('#breaking-title').val()) {
            $('#breaking-title').trigger('input');
        }
