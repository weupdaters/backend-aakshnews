...
<form id="post-create-form" class="row">
    ...
    <div class="col-xxl-4 col-xl-4 col-lg-4 mb-4">
        ...
        <div class="news-page-card">
            ...
            <!-- Thumbnails selection row (Dynamic: shows selected/uploaded photos) -->
            <div class="d-flex align-items-center gap-2 overflow-auto py-1" id="preset-thumbnails-row" style="display: none !important;">
            </div>
        </div>
        ...
        <div class="news-page-card">
            ...
            <div class="serp-card mb-3">
                ...
                <div id="serp-preview-url" class="text-truncate text-muted my-1" style="color: #0F5132; font-size: 11px;">
                    https://aakshnews.com/news/your-article-slug
                </div>
                <div id="serp-preview-desc" class="text-muted" style="font-size: 11.5px; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
                    Your article summary or meta description will appear here as a live preview for Google search engine results...
                </div>
            </div>
...
<script>
    ...
    function updateCounters() {
        ...
        $('#serp-preview-title').text(metaTitle || title || 'Your Article Title Will Appear Here');
        $('#serp-preview-desc').text(metaDesc || desc || 'Your article summary or meta description will appear here as a live preview for Google search engine results...');
    }
    ...
    $(document).ready(function() {
        ...
        $('#post-create-form').on('submit', function(e) {
            ...
            $.post('/api/posts', payload)
                 ...
                 if (res.success) {
                     if (window.Swal) {
                         Swal.fire({
                             ...
                             text: 'Your news article is now live on Aaksh News 24.',
                             timer: 1500,
                     ...
                     if (window.showAlert) {
                         window.showAlert('Review Notice', res.message || 'Please review your submission.', 'warning');
                     } else if (window.showToast) {
                         window.showToast(res.message || 'Please review submission.', 'warning');
                     }
             ...
             .fail(function(xhr) {
                 var msg = xhr.responseJSON ? xhr.responseJSON.message : 'Failed to publish article.';
                 if (window.showAlert) {
                     window.showAlert('Publish Error', msg, 'error');
                 } else if (window.showToast) {
