...
<div class="box-heading mb-4">
    <div class="box-title">
        <h3 class="mb-1">Edit Gallery Photo</h3>
        <p class="text-muted mb-0 font-sm">Modify photo name caption or update the image link.</p>
    </div>
            ...
            <div class="panel-body p-4">
                <form action="/admin/gallery/{{ $photo->id }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" class="form-control w-100 @error('name') is-invalid @enderror" placeholder="Enter photo caption or short title..." style="height: 46px; border-radius: 8px;" value="{{ old('name', $photo->name) }}" required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="border rounded p-3 mb-3 text-center" style="background:#f8fafc; min-height: 200px; display:flex; align-items:center; justify-content:center; flex-direction:column; border-color:#cbd5e1 !important;">
                            <img id="image-preview" src="{{ $photo->image_url }}" alt="Preview" style="max-height: 160px; max-width: 100%; object-fit: contain; border-radius: 6px;">
                        ...
                        <div class="input-group d-block">
                            <input type="url" name="image_url" id="gallery-image-url" class="form-control w-100 @error('image_url') is-invalid @enderror" placeholder="Or paste image URL (https://...)" style="height: 46px; border-radius: 8px;" value="{{ old('image_url', $photo->image_url) }}" required>
                            @error('image_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
